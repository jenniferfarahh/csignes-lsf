    ...
    <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-card border shadow-sm p-4 md:p-6 mb-6">
      ...
      <div className="relative z-10">
        <div className="mb-4 md:mb-6">
          ...
          <p className="text-muted-foreground text-base md:text-lg">Apprenons la LSF ensemble</p>
        </div>

        {(isLoading || isError) && (
          <Card className="p-3 mb-4">
            <p className={`text-sm ${isError ? "text-destructive" : "text-muted-foreground"}`}>
              {isLoading ? "Chargement..." : "Impossible de charger tes stats."}
            </p>
          </Card>
        )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-xs md:text-sm">
                {todayProgress === 100 ? "Bien joué 🔥" : "Fais une leçon aujourd’hui !"}
              </p>
            </div>
